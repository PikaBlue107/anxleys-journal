...
public class YoutubeUserCommentHunter {
	...
	private static final String HEADER_NOTE = "<h5>Note from Melody, the gal who created this:</h5>\n"
			+ "This paste was generated by a Java program I wrote that uses the YouTube Data API v3.\n"
			+ "Given that it's program-generated, it <b>is not perfect</b> (missing days don't mean Anxley didn't post!)\n"
...
//		Credential credential = authorize(httpTransport);
	...
	public static void main(final String[] args)
		...
		final YouTube youtubeService = getService();
		...
		final int totalComments = getCommentCount(youtubeService, DEVELOPER_KEY);
		 ...
		 */
		final YouTube.CommentThreads.List request = youtubeService.commentThreads()
		...
		commentsByDay = new ExtendableGenericArray<>(370); // use initial capacity of 370 for all 365 posts
															// + buffer
		unrecognizedComments = new ArrayList<>();
		...
		while (nextPageToken != null) {
			...
			final CommentThreadListResponse response = request.execute();
			...
			for (final CommentThread ct : response.getItems()) {
	 ...
	 */
	private static int getCommentCount(final YouTube youtubeService, final String DEVELOPER_KEY)
			throws IOException, GeneralSecurityException {
		// Define the API request
		final YouTube.Videos.List request = youtubeService.videos().list(Arrays.asList("statistics"))
	 ...
	 */
	private static void processComment(final Comment comment) {
		// Get just the top-level snippet - this contains all of the useful data like
		// author, comment text, likes, date, etc.
		final CommentSnippet topLevelSnippet = comment.getSnippet();

		// Get the Author's channel id
		final String authorChannelId = topLevelSnippet.getAuthorChannelId().getValue();
		...
		if (authorChannelId.equals(TARGET_CHANNEL_ID)) {
			...
			try {
				...
				final int day = getDayFromComment(comment);
	 ...
	 */
	private static void writeToFile(final String outfile, final int receivedComments, final int totalComments)
			throws IOException {
		...
		out.write("<p><em>Comments scanned: " + receivedComments + "/" + totalComments
				+ " total comments on video. (bc the YT Data API is buggy)</em></p>");

		// Write the current date
		out.write("<p><em>Compiled at time "
				+ DateTimeFormatter.ofPattern(DATE_TIME_FORMAT).format(LocalDateTime.now(ZoneId.of("Etc/UTC")))
				+ " GMT</em></p>");

		// Write the container divs
		...
		out.write("<div class=\"panel-heading\"><h1>Anxley's 365 Day Challenge</h1></div>");

		// Start the panel content
		...
		for (int i = 1; i < commentsByDay.length(); i++) {
			...
			final List<Comment> entries = commentsByDay.get(i);

			// If null, we haven't seen a comment for this day.
			if (entries == null) {
				// Oh no!
				out.write("<em>No comment found for day " + i + "!</em>");
			}
		...
		// End the panel body and overall panel
		out.write("</div></div>");
		...
		// Write the panel title
		out.write("<div class=\"panel-heading\">Unrecognizable comments from Anxley:</div>");
		...
		if (unrecognizedComments.size() > 0) {
			...
			for (final Comment comm : unrecognizedComments) {
		...
		// Close panel, column, row, and container
	 ...
	 */
	private static String printComment(final Comment comment) {
		...
		final Long likeCount = comment.getSnippet().getLikeCount();
		// Print out comment likes
		sb.append("\t\t\tLikes: " + likeCount + "\n");

		// Get the comment's (pretty) post date
		final String prettyDate = getPrettyDate(comment);
		// Get a URL for this comment
		final String url = getURLForComment(comment);
		// Print out URL
	 ...
	 */
	private static String getPrettyDate(final Comment comment) {
		// Format to use when reading in data
		final DateTimeFormatter iso8601Format = DateTimeFormatter.ISO_OFFSET_DATE_TIME;
		// This object stores the date and time so that we can re-format it in a nicer
		// format
		final LocalDateTime dateTimeData = iso8601Format.parse(comment.getSnippet().getPublishedAt(),
				LocalDateTime::from);
		// This is a much nicer format to view data in.
		final DateTimeFormatter niceFormat = DateTimeFormatter.ofPattern(DATE_TIME_FORMAT);
		// Convert the date and time into that nicer format
		final String dateTimeString = dateTimeData.format(niceFormat) + " GMT";
	 ...
	 */
	private static String getURLForComment(final Comment comment) {
		// All URLs are going to start with this general link
		final String prefix = "https://www.youtube.com/watch";
		// Video argument is the video ID that the comment is on
		final String video = "v=" + comment.getSnippet().getVideoId();
		// Comment argument is the id of this comment
		final String cid = "lc=" + comment.getId();
		// Stitch the URL together with ? and & syntax to give variables to the URL
		final String url = prefix + "?" + video + "&" + cid;
	 ...
	 */
	private static int getDayFromComment(final Comment comment) {
		// Get the comment string itself
		final String commentStr = comment.getSnippet().getTextDisplay();

		// Special cases
		// Day 1 has a bunch of stuff at the start before the entry itself
		if (commentStr.startsWith("Im doing t")) {
			return 1;
		}
		// Day 72 has an extra colon after "Day" and before "72"
		else if (commentStr.startsWith("Day: 72: I")) {
			return 72;
		}
		// Day 212 has an "Edit:" and an unusual format for New Year's
		else if (commentStr.startsWith("Edit: its January 12:00 am 2021 ")) {
			return 212;
		}
		...
		final int colonIdx = commentStr.indexOf(":");
		// Find the first space in the comment
		final int spaceIdx = commentStr.indexOf(" ");
		...
		try {
			...
			final String intStr = commentStr.substring(spaceIdx + 1, colonIdx);
